		...
		<div class="col-lg-9 main-chart">
			...
			<div style="margin-top: 45px;">
				...
				<form id="form_query" class="form-inline" role="form"
					action="${pageContext.servletContext.contextPath}/user/list">
					<input id="page" name="pageNo" type="hidden" value="1" />
					<div class="form-group">
						<label class="sr-only" for="exampleInputEmail2">Email
							address</label> <input type="text" class="form-control"
							id="exampleInputEmail2" placeholder="User Name" name="username"
							value="${user.username}">
					</div>
					<div class="form-group">
						<label class="sr-only" for="exampleInputPassword2">Password</label>
						<input type="text" class="form-control" id="exampleInputPassword2"
							placeholder="User Status" name="status" value="${user.status}">
					</div>
			...
			<div class="content-panel">
				<table class="table table-striped table-advance table-hover">
					...
					<thead>
						<tr>
							<th style="padding-left:50px"><i class="fa fa-bullhorn"></i>登录名</th>
							<th class="hidden-phone" style="padding-left:50px"><i class="fa fa-question-circle"></i>
								用户名</th>
							<th style="padding-left:50px"><i class="fa fa-bookmark"></i> 状态</th>
					...
					<tbody>
						<!-- 遍历用户 -->
						<c:forEach items="${pageInfo.list}" var="userItem">
							<tr>
								<td style="padding-left:50px">${userItem.loginname}</td>
								<td class="hidden-phone" style="padding-left:50px">${userItem.username}</td>
								<td style="padding-left:50px">
									<c:if test="${userItem.status==1}">超级管理员</c:if>
									<c:if test="${userItem.status==0}">管理员</c:if>
								</td>
								<td style="padding-left:50px"><span class="label label-info label-mini"> <fmt:formatDate
											value="${userItem.createdate}" pattern="yyyy-MM-dd HH:mm:ss" />
								</span></td>
								<c:if test="${nowUser.status==1}">
									<td style="padding-left:50px">
										<button class="btn btn-primary btn-xs">
											<a class="fa fa-pencil" style="color: white;" href="${pageContext.servletContext.contextPath}/user/get?id=${userItem.id}">Update</a>
										</button>
										<button class="btn btn-danger btn-xs" onclick="">
											<a class="fa fa-trash-o" style="color: white;" href="${pageContext.servletContext.contextPath}/user/delete?id=${userItem.id}">Delete</a>
										</button>
									</td>
								</c:if>
							</tr>
